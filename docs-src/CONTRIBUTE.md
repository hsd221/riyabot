...
贡献前，请先确认改动范围、目标分支和验证方式。RiyaBot 的正常改动必须先进入 `dev`；验证通过后，再通过 Pull Request 合并到 `main`。
...
提交日志前，必须删除 Token、Cookie、QQ 号、API Key、模型响应中的隐私内容和其他凭据。不要把“可能是某个外部服务导致”当作结论，除非日志或对照测试能够支持这一判断。

## 选择改动入口

- 文档错误、示例修正或文档站调整，可以直接提交文档改动。
...
1. 从当前 `dev` 创建工作分支。
2. 在工作分支中完成范围明确的改动并推送，然后通过 Pull Request 合入 `dev`。只有得到明确授权，才可以直接提交 `dev`。
3. 在 `dev` 上完成验证后，再通过 Pull Request 合并到 `main`。

普通改动不得直接提交或合并到 `main`。提交信息使用 Conventional Commits 前缀，例如 `feat:`、`fix:`、`docs:`、`refactor:` 或 `chore:`。

### 紧急改动

只有重要修复或热补丁可以直接更新 `main`。完成后，必须立即将同一改动同步回 `dev`，避免两个分支继续分叉。

### 版本和发布

- 正式版本号、正式 Tag 和正式 Release 只在 `main` 上维护。
- `dev` 使用带后缀的预发布版本号，发布类型为 Pre-release。
- 发布前，请确认版本号、Tag 可达分支和 Release 类型彼此一致。
...
先查看工作树，确认提交中没有包含其他人的改动、运行时文件或生成产物：
...
- `webui/dist/`、`webui/node_modules/`、`docs-src/.vitepress/dist/`、`docs-src/node_modules/` 和缓存；
