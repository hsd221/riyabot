...
源码安装适合需要修改 RiyaBot、开发插件或参与 WebUI 开发的场景。`main` 分支已内置构建好的 WebUI，你只需安装 Python 依赖，然后由 `python bot.py` 同时启动后端和管理面板。

如果你不想在宿主机安装 Python，请改看 [Docker 安装](docker-installation.md)。
...
后端会从 `webui/dist/` 托管前端静态文件。**`main` 分支的 `webui/dist/` 已由 CI 构建并提交到仓库**，克隆 `main` 分支后开箱即用，无需手动构建。

只有以下情况需要手动构建：

- 你检出的是 `dev` 分支（`webui/dist/` 不进仓库）；
- 你需要修改 WebUI 前端代码。
...
1. 确认 EULA 和隐私协议；
2. 填写 bot 身份信息（平台、账号、昵称）；
3. 在模型管理中填写模型供应商、模型和 API Key，并完成任务分配。

至少需要配置一个用于回复的语言模型；启用记忆、识图、语音等功能还需相应的 embedding / VLM / ASR 模型。

详细配置项见[配置指南](configuration.md)。

## 6. 连接聊天平台

RiyaBot 通过适配器（Adapter）连接消息平台。适配器与核心之间使用旧版消息 WebSocket 通信，默认在 `127.0.0.1:8000`（由 `.env` 的 `HOST` / `PORT` 控制）。跨主机连接时需要为核心与适配器配置同一个令牌，详见[部署指南](deployment.md)。
...
改动生效后，如需让 `python bot.py` 托管更新后的前端，重新构建生产静态资源即可：
