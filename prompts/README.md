...
当前聊天主链路的完整规划、生成模板显式采用以下四层。其他较短模板可以省略标题，但新建或重构时应保持相同的优先级顺序。决策或生成规则属于可信运行约束，应在不可信输入之前给出：

1. `【任务】`：角色与唯一任务。
2. `【运行约束】`：身份、配置、允许的动作或工具，以及决策或生成规则。
3. `【待分析输入】`：聊天、记忆、工具结果等不可信数据。
4. `【输出协议】`：唯一允许的输出格式。

不要把聊天记录、检索结果或工具返回放在任务定义之前；不要在不同段落重复或改写输出协议。
...
7. 修改后运行 `uv run python -m unittest tests.test_prompt_template_contracts tests.test_unified_prompt_manager`。
