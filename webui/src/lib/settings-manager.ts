...
export const STORAGE_KEYS = {
  ...
  ENABLE_WAVES_BACKGROUND: 'riyabot-waves-background',
...
export function getSetting<K extends keyof Settings>(key: K): Settings[K] {
  ...
  const legacyTheme = key === 'theme' ? localStorage.getItem(STORAGE_KEYS.LEGACY_THEME) : null
  ...
  if (key === 'theme') {
    if (isThemeValue(stored)) {
      if (currentValue === null) {
        localStorage.setItem(STORAGE_KEYS.THEME, stored)
      }
      if (legacyTheme !== null) {
...
export function setSetting<K extends keyof Settings>(key: K, value: Settings[K]): void {
  ...
  // 触发自定义事件，通知其他组件设置已更新
  window.dispatchEvent(new CustomEvent('riyabot-settings-change', {
    detail: { key, value }
  }))
}
...
export function resetAllSettings(): void {
  ...
  // 触发全局事件
  window.dispatchEvent(new CustomEvent('riyabot-settings-reset'))
}
