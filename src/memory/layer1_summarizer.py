...
class GroupTopicSummarizer:
    ...
    def get_topic_summaries(self, stream_id: str) -> list[dict]:
        """获取指定流的当前所有话题摘要
...
class UnclosedTopicBridge:
    ...
    def save_unclosed_topics(self, stream_id: str, topics: list[dict]) -> None:
        """保存未闭合话题
    ...
    def restore_topics(self, stream_id: str) -> list[dict]:
        """恢复未闭合话题并从存储中清除
