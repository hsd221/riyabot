...
class Waiter:
    ...
    async def wait(self, conversation_info: ConversationInfo) -> bool:
        ...
        logger.debug(f"[私聊][{self.private_name}]进入常规等待状态 (超时: {DESIRED_TIMEOUT_SECONDS} 秒)")

        while True:
            # 检查是否有新消息
            if self.chat_observer.new_message_after(wait_start_time):
                logger.debug(f"[私聊][{self.private_name}]等待结束，收到新消息")
                return False  # 返回 False 表示不是超时

            # 检查是否超时
            elapsed_time = time.time() - wait_start_time
            if elapsed_time > DESIRED_TIMEOUT_SECONDS:
                logger.debug(f"[私聊][{self.private_name}]等待超时，添加目标")
                wait_goal = {
                ...
                logger.debug(f"[私聊][{self.private_name}]添加等待目标")
    ...
    async def wait_listening(self, conversation_info: ConversationInfo) -> bool:
        """倾听用户发言或超时"""
        wait_start_time = time.time()
        logger.debug(f"[私聊][{self.private_name}]进入倾听等待状态 (超时: {DESIRED_TIMEOUT_SECONDS} 秒)")

        while True:
            # 检查是否有新消息
            if self.chat_observer.new_message_after(wait_start_time):
                logger.debug(f"[私聊][{self.private_name}]倾听等待结束，收到新消息")
                return False  # 返回 False 表示不是超时

            # 检查是否超时
            elapsed_time = time.time() - wait_start_time
            if elapsed_time > DESIRED_TIMEOUT_SECONDS:
                logger.debug(f"[私聊][{self.private_name}]倾听等待超时，添加目标")
                wait_goal = {
                    # 保持 goal 文本一致
                ...
                conversation_info.goal_list.append(wait_goal)
                logger.debug(f"[私聊][{self.private_name}]添加倾听等待目标")
                return True  # 返回 True 表示超时
