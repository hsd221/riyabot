...
def _has_api_server_support() -> bool:
    try:
        from maim_message import MessageConverter  # noqa: F401
        from maim_message.client import WebSocketClient, create_client_config  # noqa: F401
        from maim_message.message import APIMessageBase  # noqa: F401
    except ImportError:
        return False
    return True
...
try:
    maim_message_version = importlib.metadata.version("maim-message")
    version_int = [int(x) for x in maim_message_version.split(".")]
    HAS_MESSAGE_CONVERTER = version_int >= [0, 6, 2] and _has_api_server_support()
...
class APIServerWrapper:
    ...
    async def send_message(self, message: MessageBase) -> bool:
        ...
        # 接收场景：Adapter 收到来自 Napcat 的消息，发送给 RiyaBot
...
async def _legacy_message_handler_adapter(message: Any, metadata: dict):
    ...
    data = {
        ...
        "message_segment": {"type": message.message_segment.type, "data": message.message_segment.data},
    }
    ...
    # mmc_com_layer seems to be for Adapter talking to RiyaBot Core.
    # recv_handler/message_sending.py uses this router to send TO RiyaBot.
    # The `register_class_handler` in `mmc_start_com` suggests RiyaBot sends messages TO Adapter?
    # Wait, `send_handler.handle_message` seems to be handling messages RECEIVED FROM RiyaBot.
...
async def mmc_start_com():
    ...
    if config.enable_api_server and HAS_MESSAGE_CONVERTER:
        ...
        if config.enable_api_server and not HAS_MESSAGE_CONVERTER:
            logger.warning("当前 maim-message 版本不支持 API-Server 模式，回退 Legacy WebSocket 模式")
        logger.info("使用 Legacy WebSocket 模式连接 RiyaBot")
