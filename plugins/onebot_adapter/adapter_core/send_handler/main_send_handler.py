...
class SendHandler:
    ...
    async def send_command(self, raw_message_base: MessageBase) -> None:
        ...
        if not command or not args_dict:
            ...
            await self._send_command_response(
                platform=message_info.platform, command_name=command_name, success=False, error="命令或参数缺失"
            )
            return None
    ...
    async def _send_command_response(
        self, platform: str, command_name: str, success: bool, data: Optional[Dict] = None, error: Optional[str] = None
    ) -> None:
        """发送命令响应回璃夜
        ...
        Args:
            ...
            error: 错误信息（失败时）
        """
        response_data = {"command_name": command_name, "success": success, "timestamp": time.time()}
        ...
        try:
            await message_send_instance.send_custom_message(
                custom_message=response_data, platform=platform, message_type="command_response"
            )
