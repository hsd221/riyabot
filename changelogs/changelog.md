...
### 🌟 重大更新
- 记忆子系统支持行为开关：梦境维护、遗忘扫描、图谱关联扩展均可在 `[memory]` 配置中独立启停，并同步反映到梦境周期内的遗忘阶段。
- 检索链路接通梦境实体图谱：梦境周期构建的知识图谱现在通过进程级共享 GraphStore 参与回复检索的关联扩展，不再只建不用。
- 梦境周期接通数据维护：日常周期清理过期原始消息归档，每周周期修剪低权重原子关联，归档表与关联表不再无限增长。

### 细节功能更改
- 修复 e2e 工具链三处失配：健康检查改为从模型配置派生服务地址、就绪标记对齐当前启动日志、模拟器注入消息改用实时时间戳，`run_e2e --quick` 全流程可跑通并以真实请求验证记忆归档、编码与向量写入链路。

## [0.14.1-dev.1] - 2026-08-08
### 🌟 重大更新
- 聊天记录导入支持配置提取并发，并保留有序提交与断点恢复。

## [0.14.0] - 2026-08-07
### 🌟 重大更新
- WebUI 支持检查远端正式版标签或开发分支提交，并可在满足条件时由 Runner 执行一键更新。
- 新增更新频道切换、更新结果反馈和 Docker/压缩包安装模式的安全提示。
- 首页版本号会提示当前频道的可用更新，并可直接跳转到系统更新页面。
- 用 VitePress 搭建文档站并配置 GitHub Pages 自动部署，源码安装与部署、插件开发、配置说明集中收录。
- `main` 分支跟踪构建后的 `webui/dist`，源码安装克隆仓库即得构建好的 WebUI 静态资源。
- 全站文档措辞修订：统一「聊天机器人」表述、重写部署指南、简化贡献指南、首页系统结构展示项目特色。

## [0.13.0] - 2026-07-03
### 🌟 重大更新
- 重构记忆系统为 L0-L3 四层漏斗架构，覆盖归档、摘要、LLM 编码、多路检索与 Prompt 组装。
- 新增 `src/memory/` 记忆模块体系，包含 SQLite/Qdrant 双存储、写操作日志、遗忘衰减、用户画像、图检索、梦境维护与冲突仲裁。
- Qdrant 支持本地嵌入式模式，失败时自动降级，不再要求额外 Docker 服务。

### 细节功能更改
- 修复 Qdrant point id、payload 同步、检索扩展、GraphStore、噪声记录、异常隔离等记忆系统问题。
- 将记忆存储配置迁移到全局 TOML，并增加 SQLite/Qdrant 双写一致性修复任务。
- 修复模型配置页面、记忆 WebUI 展示、搜索对话框死链等问题。
- 移除旧 LPMM 知识图谱相关配置页与插件示例残留。
